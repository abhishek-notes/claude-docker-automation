...
app.post('/api/launch-task', async (req, res) => {
    try {
        const { projectPath, taskContent, sessionId, itermProfile, useDocker, openInNewTab } = req.body;
...
async function launchClaudeSession(projectPath, sessionId, itermProfile = 'Default', useDocker = false, openInNewTab = true) {
    return new Promise((resolve, reject) => {
        ...
        const sessionEnv = { 
            ...
            CLAUDE_OPEN_NEW_TAB: openInNewTab ? 'true' : 'false',
            CLAUDE_AUTO_PASTE: 'true'  // Enable intelligent auto-paste that waits for Claude prompt
        };
        ...
        if (useDocker) {
            ...
            // Launch tmux session with Claude automation directly (wrapper causing issues)
            const scriptPath = path.join(__dirname, 'claude-tmux-launcher.sh');
            childProcess = spawn(scriptPath, [projectPath, 'CLAUDE_TASKS.md'], {
                cwd: __dirname,
                env: sessionEnv,
                stdio: ['pipe', 'pipe', 'pipe'],
                detached: true
